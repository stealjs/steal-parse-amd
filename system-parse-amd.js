...
function amd(loader) {
	...
	function getFactoryText(source, lastMatches, lastIndex) {
		...
		while(openBrackets && index < source.length) {
		...
		return source.substring(startArgs, index);
	}
	
	function getAMDDeps(source) {
		console.log(source);
		...
		while(res = amdRegEx.exec(source)) {
			console.log(res);
			var name = trimFirstAndLast( res[1] ),
				deps = getArgs( res[2] );
			...
			amdRegEx.lastIndex = amdRegEx.lastIndex+res[0].length;
		}
	...
	*/
	...
	loader.instantiate = function(load) {
		...
		if(load.metadata.format === 'amd' ) {
			load.metadata.deps = load.metadata.deps ? load.metadata.deps.concat(getAMDDeps(load.source)) : getAMDDeps(load.source);
